...
class DDPG(RLAlgorithm):
    ...
    def train(self):
        with tf.Session() as sess:
            ...
            novice_only_pool = SimpleReplayPool(
                max_pool_size=self.replay_pool_size,
                observation_dim=self.env.observation_space.flat_dim,
                action_dim=self.env.action_space.flat_dim,
                replacement_prob=self.replacement_prob,
            ...
            with tf.variable_scope("sample_policy"):
                ...
                #addition here
                self.oracle_sample_policy = Serializable.clone(self.oracle_policy)
            ...
            for epoch in range(self.n_epochs):
                ...
                for epoch_itr in pyprind.prog_bar(range(self.epoch_length)):
                    ...
                    oracle_action = self.get_oracle_action(itr, observation, policy=self.oracle_sample_policy )
                    ...
                    # TODO: this is a hack! What you should do is just probably store the sigma value and then select all oracle/etc. based on that
                    ####################
                    if sigma < 1:
                        novice_only_pool.add_sample(observation, action, reward * self.scale_reward, terminal, initial)
                    else:
                    ...
                    if pool.size >= self.min_pool_size:
                        for update_itr in range(self.n_updates_per_sample):
                            ...
                            novice_batch = novice_pool_only.random_batch(self.batch_size)
                            itrs = self.do_training(itr, batch, oracle_batch, novice_batch)
                            train_qf_itr += itrs[0]
    ...
    def init_opt(self):
        ...
        #TODO: not sure if the below is a principled update rule
    ...
    def do_training(self, itr, batch, oracle_batch, novice_batch):
        ...
        self.f_train_gf(obs, oracle_actions, ys_oracle, obs, ys, obs, actions
        """
        obs_novice_only, actions_novice_only, rewards_novice_only, next_obs_novice_only, terminals_novice_only = ext.extract(
            novice_batch,
            "observations", "actions", "rewards", "next_observations",
            "terminals"
        )
        ...
        next_actions_novice_only, _ = target_policy.get_actions(next_obs_novice_only)
        next_qvals_novice_only = target_qf.get_qval(next_obs_novice_only, next_actions_novice_only)
        ys_novice_only = rewards_novice_only + (1. - terminals_novice_only) * self.discount * next_qvals_novice_only.reshape(-1)

        next_actions_oracle_only, _ = target_policy.get_actions(next_obs_oracle_only)
        ...
        gf_loss, gval, _ = self.f_train_gf(obs_oracle_only, actions_oracle_only, ys_oracle_only, ys_novice_only, obs_novice_only, actions_novice_only )

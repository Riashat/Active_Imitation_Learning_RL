...
class DDPG(RLAlgorithm):
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(self.n_epochs):
                ...
                for epoch_itr in pyprind.prog_bar(range(self.epoch_length)):
                    ...
                    # TODO: FINISH this, add required params
                    ########################################################
                    es_action = self.es.get_action(itr, observation, policy=sample_policy)  # qf=qf)

                    oracle_action = self.oracle.get_action(observation)

                    sigma = round(self.gating_function(observation)) # hard gate for now

                    action =  sigma * oracle_action + (1 - sigma) * es_action
                    ###############################################################
    ...
    def init_opt(self):
        ...
        ### TODO::: modified here
        ################################################
        # TODO: maybe this should actually be based on the uncertainty of the action!
        # then you combine the bayesian stuff in and then just query the oracle when you don't know what to do!
        ...
        self.gating_function = tf.reshape(gating_func_net.output, [-1])
        ...
        self.policy_update_method.update_opt(gf_loss, target=self.gating_function, inputs=gf_input_list )

        self.f_train_gf = tensor_utils.compile_function(
            inputs=gf_input_list,
            outputs=[gf_loss, self.gf_update_method._train_op],
        )
    ...
    def do_training(self, itr, batch):
        ...
        #TODO: add call to training func here for gating_func
        # ==========-===================================
